...
class Actor(nn.Module):
    ...
    def forward(self, s):
        ...
        mu = self.mu(s)
        log_std = torch.clamp(self.log_std(s), -20, 2)
        return mu, log_std
    ...
    def sample(self, s):
        ...
        log_prob = dist.log_prob(x_t) - torch.log(self.max_action * (1 - y_t.pow(2))+1e-6)
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.layer_4 = nn.Linear(state_dim, 256)
...
class SAC(object):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.alpha_optimizer = torch.optim.Adam([self.log_alpha], lr=self.critic_optimizer.param_groups[0]['lr'])
        self.max_action = max_action
    ...
    def train(
        ...
        for it in range(iterations):
            ...
            target_Q = reward + ((1 - done) * discount * target_V).detach()
        ...
        env.get_logger().info(f"actor_loss, alpha_loss, critic_loss, Av.Q, Max.Q, iterations : {av_actor_loss / iterations},{av_alpha_loss / iterations} ,{av_critic_loss / iterations}, {av_Q / iterations}, {max_Q}, {self.iter_count}")
        self.writer.add_scalar("actor_loss", av_actor_loss / iterations, self.iter_count)
        self.writer.add_scalar("alpha_loss", av_alpha_loss / iterations, self.iter_count)
    ...
    def save(self, filename, directory):
        ...
        save_path = os.path.join(cwd, directory)
        ...
        actor_path = os.path.join(save_path, f"{filename}_actor.pth")
        critic_path = os.path.join(save_path, f"{filename}_critic.pth")
...
class Image_subscriber(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(
            Image,
            '/front_camera/image_raw',  # 確認 Topic 名稱是否與 URDF 一致
            self.image_callback,
            10)
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # 縮放至 ViT 常用尺寸 (224x224)
            latest_camera_image = cv2.resize(cv_img, (224, 224))
